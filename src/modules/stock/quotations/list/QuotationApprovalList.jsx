...
const quotationCategories = (q) => {
  const map = new Map();
  (q.items || []).forEach((it) => { const path = it.categoryName || ""; if (path && !map.has(path)) map.set(path, { leaf: leafOf(path), path }); });
  return [...map.values()];
};
const CategoriesCell = ({ q, onMore }) => {
  ...
  return (
    <div className="flex flex-wrap items-center gap-1">
      {shown.map((c, i) => <span key={i} className="rounded-full bg-gray-100 px-2 py-0.5 text-xs text-gray-700">{c.leaf}</span>)}
      {extra > 0 && <button type="button" onClick={() => onMore(cats)} className="rounded-full bg-indigo-50 px-2 py-0.5 text-xs font-medium text-indigo-600 hover:bg-indigo-100">+{extra}</button>}
    </div>
  );
};
...
const QuotationApprovalList = () => {
  ...
  return (
    <div className="p-6">
      <div className="mb-4">
        <h1 className="text-xl font-semibold text-gray-900">Quotation approvals</h1>
        <p className="mt-1 text-sm text-gray-500">Review pending quotations category-wise</p>
      </div>
      ...
      <div className="mb-4 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
        ...
        <div className="mt-3 flex flex-wrap items-end gap-3">
          <div>
            <label className="mb-1 block text-[11px] uppercase text-gray-500">{status === "PENDING" || status === "" ? "Submitted from" : "Decided from"}</label>
            <input type="date" value={dateFrom} onChange={(e) => { setDateFrom(e.target.value); setPage(1); }} className="rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
          </div>
          <div>
            <label className="mb-1 block text-[11px] uppercase text-gray-500">To</label>
            <input type="date" value={dateTo} onChange={(e) => { setDateTo(e.target.value); setPage(1); }} className="rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
          </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-gray-200 bg-white">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              <tr><td colSpan={6} className="px-4 py-12 text-center"><div className="mx-auto h-8 w-8 animate-spin rounded-full border-2 border-indigo-600 border-t-transparent" /><p className="mt-2 text-sm text-gray-500">Loading…</p></td></tr>
            ) : rows.length === 0 ? (
                  ...
                  <tr key={q._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button type="button" onClick={() => router.push(`/stock/quotations/${q._id}/details`)} className="rounded-md border border-gray-200 px-3 py-1.5 text-xs font-medium text-gray-600 hover:bg-gray-50">Details</button>
                        {isPending ? (
                          <button type="button" onClick={() => router.push(`/stock/quotations/${q._id}/review`)} className="rounded-md border border-amber-200 px-3 py-1.5 text-xs font-medium text-amber-700 hover:bg-amber-50">Review</button>
                        ) : (
                          <button type="button" onClick={() => router.push(`/stock/quotations/${q._id}/view`)} className="rounded-md border border-indigo-200 px-3 py-1.5 text-xs font-medium text-indigo-600 hover:bg-indigo-50">View</button>
                        )}
                      </div>
                    </td>
                  </tr>
      ...
      {moreCats && (
        <Modal onClose={() => setMoreCats(null)} title="Categories">
          <div className="space-y-2">
            {moreCats.map((c, i) => (
              <div key={i} className="rounded-lg border border-gray-100 px-3 py-2"><p className="text-sm font-medium text-gray-900">{c.leaf}</p><p className="text-xs text-gray-500">{c.path}</p></div>
            ))}
          </div>
        </Modal>
      )}
